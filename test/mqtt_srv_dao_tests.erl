...
%	?debug_Fmt("::test:: read returns R2a ~120p", [R2a]),	
	...
	?assertEqual(#{password => mqtt_data:binary_to_hex(crypto:hash(md5, <<"aaaaaaa">>)), roles => ["USER", "ADMIN", "OWNER"]}, R3),
	R3a = Storage:get(server, {user_id, <<"alex">>}),
	?debug_Fmt("::test:: read get User Info returns R3a ~120p", [R3a]),	
	?assertEqual(#{password => mqtt_data:binary_to_hex(crypto:hash(md5, <<"aaaaaaa">>)), roles => ["USER", "ADMIN", "OWNER"]}, R3a),
