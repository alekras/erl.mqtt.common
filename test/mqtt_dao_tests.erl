...
create(X, Storage) -> {"create [" ++ atom_to_list(X) ++ "]", timeout, 1, fun() ->
	Storage:save(client, #storage_publish{key = #primary_key{client_id = "lemon", packet_id = 101}, document = #publish{topic = "AK", payload = <<"Payload lemon 1">>}}),
 	...
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "AKtest", client_id = "lemon"}, options = #subscription_options{max_qos=0}, callback = {erlang, timestamp}}),
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "Winter/+", client_id = "orange"}, options = #subscription_options{max_qos=1}, callback = {mqtt_client_test, callback}}),
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "+/December", client_id = "apple"}, options = #subscription_options{max_qos=2}, callback = {length}}),
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "Winter/#", client_id = "pear"}, options = #subscription_options{max_qos=1}, callback = {length}}),
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "Winter/+/2", client_id = "plum"}, options = #subscription_options{max_qos=2}, callback = {length}}),
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "/+/December/+", client_id = "orange"}, options = #subscription_options{max_qos=2}, callback = {length}}),
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "+/December", client_id = "orange"}, options = #subscription_options{max_qos=0}, callback = {size}}),
 	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "+/December/+", client_id = "apple"}, options = #subscription_options{max_qos=0}, callback = {length}}),
...
%	?debug_Fmt("::test:: read returns R1 ~120p", [R1]),	
 	?assertEqual(#storage_subscription{key = #subs_primary_key{topic = "AKtest", client_id = "lemon"}, options = #subscription_options{max_qos=0}, callback = {erlang, timestamp}}, R1),
...
%	?debug_Fmt("::test:: read returns ~120p", [R]),	
	?assert(lists:member({"+/December",#subscription_options{max_qos=0},{size}}, R)),
	?assert(lists:member({"/+/December/+",#subscription_options{max_qos=2},{length}}, R)),
	?assert(lists:member({"Winter/+",#subscription_options{max_qos=1},{mqtt_client_test, callback}}, R)),
	?passed
end}.
...
%	?debug_Fmt("::test:: read returns ~120p", [R]),	
	?assertEqual([{"+/December",#subscription_options{max_qos=0},{size}},
								{"Winter/+",#subscription_options{max_qos=1},{mqtt_client_test, callback}}], R),

	R1 = Storage:get_matched_topics(client, "Winter/December"),
%	?debug_Fmt("::test:: read returns ~120p", [R1]),	
	?assertEqual([{storage_subscription,{subs_primary_key,"+/December","apple"},#subscription_options{max_qos=2},{length}},
								{storage_subscription,{subs_primary_key,"+/December","orange"},#subscription_options{max_qos=0},{size}},
								{storage_subscription,{subs_primary_key,"Winter/#","pear"},#subscription_options{max_qos=1},{length}},
								{storage_subscription,{subs_primary_key,"Winter/+","orange"},#subscription_options{max_qos=1},{mqtt_client_test, callback}}], R1),
	?passed
end}.
...
%	?debug_Fmt("::test:: read returns ~120p", [R1]),
	...
	Storage:save(client, #storage_subscription{key = #subs_primary_key{topic = "Winter/+", client_id = "orange"}, options = #subscription_options{max_qos=2}, callback = {erlang, binary_to_list}}),
