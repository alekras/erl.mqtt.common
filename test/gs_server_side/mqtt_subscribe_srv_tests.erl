...
storage_test('3.1.1'=Version, {Socket, Conn_config}) -> {"Storage test [" ++ atom_to_list(Version) ++ "]", timeout, 10, fun() ->
	...
	Storage = storage_module(application:get_env(mqtt_common, storage, dets)),
	?debug_Fmt("::test:: Records = ~p ~n", [Storage:subscription(get_client_topics,<<"test0Client">>,server)]),
	[Record] = Storage:subscription(get, #subs_primary_key{client_id= <<"test0Client">>, topicFilter= "Topic"}, server),
	...
	Record1 = Storage:subscription(get, #subs_primary_key{client_id= <<"test0Client">>, topicFilter= "Topic"}, server),
	?assertEqual([], Record1),
...
%%	subscribe(Version, Socket, {0,0,0,2}),
	...
	Storage = storage_module(application:get_env(mqtt_common, storage, dets)),
	?debug_Fmt("::test:: Records = ~p ~n", [Storage:subscription(get_client_topics,<<"test0Client">>,server)]),
	[Record] = Storage:subscription(get, #subs_primary_key{client_id= <<"test0Client">>, topicFilter= "TopicA", shareName= "A"}, server),
	?debug_Fmt("::test:: Record = ~p ~n", [Record]),
	?assertEqual(2, (Record#storage_subscription.options)#subscription_options.max_qos),	
...
%%	subscribe(Version, Socket, {0,0,0,2}),
	...
	Storage = storage_module(application:get_env(mqtt_common, storage, dets)),
	?debug_Fmt("::test:: Records = ~p ~n", [Storage:subscription(get_client_topics,<<"test0Client">>,server)]),
	[] = Storage:subscription(get, #subs_primary_key{client_id= <<"test0Client">>, topicFilter= "TopicA", shareName= "A"}, server),
...
retain0_test('3.1.1' = Version, {Socket, Conn_config}) -> {"Retain test [" ++ atom_to_list(Version) ++ "]", timeout, 10, fun() ->
	...
	timer:sleep(100),
	Storage = storage_module(application:get_env(mqtt_common, storage, dets)),
	?debug_Fmt("::test:: Records = ~p ~n", [Storage:retain(get_all,"Topic")]),
	[Record] = Storage:retain(get, "Topic"),
	?debug_Fmt("::test:: Record = ~p ~n", [Record]),
...
retain3_test('5.0' = Version, {Socket, Conn_config}) -> {"Retain2 test [" ++ atom_to_list(Version) ++ "]", timeout, 10, fun() ->
	...
	timer:sleep(100),
	Storage = storage_module(application:get_env(mqtt_common, storage, dets)),
	?debug_Fmt("::test:: Records = ~p ~n", [Storage:retain(get_all,"TopicA")]),
	[Record] = Storage:retain(get, "TopicA"),
	?debug_Fmt("::test:: Record = ~p ~n", [Record]),
