...
subscribe_test('3.1.1'=Version, Conn_config) -> {"Subscribe test [" ++ atom_to_list(Version) ++ "]", timeout, 5, fun() ->
	...
	conn_server ! {tcp, undefined, <<130,10,0,100,0,5,84,111,112,105,99,2>>}, %% Subscription request
...
unsubscribe_test('3.1.1'=Version, Conn_config) -> {"Unsubscribe test [" ++ atom_to_list(Version) ++ "]", timeout, 5, fun() ->
	...
	conn_server ! {tcp, undefined, <<162,9,0,101,0,5,84,111,112,105,99>>}, %% Unsubscription request
...
unsubscribe_test('5.0' = Version, Conn_config) -> {"Unsubscribe test [" ++ atom_to_list(Version) ++ "]", timeout, 1, fun() ->
	...
	conn_server ! {tcp, undefined, <<162,10,0,101,0,0,5,84,111,112,105,99>>}, %% Unsubscription request
	wait_mock_tcp("unsuback"),
...
publish_0_test('3.1.1'=Version, Conn_config) -> {"Publish 0 test [" ++ atom_to_list(Version) ++ "]", timeout, 5, fun() ->
	...
	conn_server ! {tcp, undefined, <<48,14,0,5,84,111,112,105,99,80,97,121,108,111,97,100>>}, %% Publish packet from client -> server
...
publish_0_test('5.0' = Version, Conn_config) -> {"Publish 0 test [" ++ atom_to_list(Version) ++ "]", timeout, 1, fun() ->
	...
	conn_server ! {tcp, undefined, <<48,15,0,5,84,111,112,105,99,0,80,97,121,108,111,97,100>>}, %% Publish packet from client -> server
	wait_mock_tcp("publish<0>"),
...
publish_0_props_test('5.0' = Version, Conn_config) -> {"Publish 0 test [" ++ atom_to_list(Version) ++ "]", timeout, 1, fun() ->
	...
	conn_server ! {tcp, undefined, <<48,25,0,5,84,111,112,105,99, 10, 9,4:16,1,2,3,4, 35,300:16, 80,97,121,108,111,97,100>>}, %% Publish packet from client -> server
	wait_mock_tcp("publish<0>"),
