...
publish_1_test('3.1.1'=Version, Conn_config) -> {"Publish 1 test [" ++ atom_to_list(Version) ++ "]", timeout, 5, fun() ->
	...
	{ok, _Ref} = gen_server:call(client_gensrv, {publish, #publish{qos = 1, topic = <<"Topic">>, payload = <<"Payload">>}}),
...
publish_1_props_test('5.0' = Version, Conn_config) -> {"Publish 1 test [" ++ atom_to_list(Version) ++ "]", timeout, 1, fun() ->
	...
	{ok, _Ref} = gen_server:call(client_gensrv, {publish,
...
publish_2_test('3.1.1'=Version, Conn_config) -> {"Publish 2 test [" ++ atom_to_list(Version) ++ "]", timeout, 5, fun() ->
	...
	{ok, _Ref} = gen_server:call(client_gensrv, {publish, #publish{qos = 2, topic = <<"Topic">>, payload = <<"Payload">>}}),
...
publish_2_props_test('5.0' = Version, Conn_config) -> {"Publish 2 test [" ++ atom_to_list(Version) ++ "]", timeout, 1, fun() ->
	...
	{ok, _Ref} = gen_server:call(client_gensrv, {publish, #publish{qos = 2, topic = <<"Topic">>,
																															payload = <<"Payload">>,
																															properties = [{?Topic_Alias, 300},{?Correlation_Data, <<1,2,3,4>>}]}}),
	wait_mock_tcp("publish<2> packet"),
...
%% from server:
	Conn_State = sys:get_state(client_gensrv),
	client_gensrv ! {tcp, Conn_State#connection_state.socket, Connack_packet}, %% Connack packet
	receive
		[onConnect, _] = _Args ->
...
%% from server:
	client_gensrv ! {tcp, (sys:get_state(client_gensrv))#connection_state.socket, Suback_packet}, %% Suback packet
	receive
...
%%	from server
	Conn_State = sys:get_state(client_gensrv),
	client_gensrv ! {tcp, Conn_State#connection_state.socket, <<224,0>>}, %% Disconnect packet
	receive
