	...
	[{ setup,
		...
		{ foreachx,
			...
			[
				...
				,{'3.1.1', fun restore_session_test/2}
				,{'5.0',   fun restore_session_test/2}
...
restore_session_test('3.1.1' = Version, Conn_config) -> {"Restore session test [" ++ atom_to_list(Version) ++ "]", timeout, 5, fun() ->
	?debug_Fmt("::test:: >>> test(~p, ~p) test process PID=~p~n", [Version, Conn_config, self()]),
...
restore_session_test('5.0' = Version, Conn_config) -> {"Restore session test [" ++ atom_to_list(Version) ++ "]", timeout, 5, fun() ->
	?debug_Fmt("::test:: >>> test(~p, ~p) test process PID=~p~n", [Version, Conn_config, self()]),
	connect(Version, Conn_config#connect{properties = [{?Topic_Alias_Maximum, 2},{?Receive_Maximum, 10}]}),
	State = sys:get_state(client_gensrv),
	?debug_Fmt("::test:: State = ~p ~n", [State]),
...
connect('5.0', Conn_config) ->
	...
	Connack_packet = <<32,9,1,0,6,33,10:16,34,2:16>>,
	connect(Conn_config, Expected_packet, Connack_packet).
