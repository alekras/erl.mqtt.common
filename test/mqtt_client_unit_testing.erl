...
decode_remaining_length() ->
	?assertEqual({<<1, 1>>, 2}, mqtt_input:decode_remaining_length(<<2:8, 1:8, 1:8>>)),
	?assertEqual({<<1, 1>>, 2049}, mqtt_input:decode_remaining_length(<<16#81:8, 16:8, 1:8, 1:8>>)),
	?assertEqual({<<1, 1>>, 47489}, mqtt_input:decode_remaining_length(<<16#81:8, 16#f3:8, 2:8, 1:8, 1:8>>)),
	?assertEqual({<<1, 1>>, 32110977}, mqtt_input:decode_remaining_length(<<16#81:8, 16#f3:8, 16#A7, 15:8, 1:8, 1:8>>)),
	?passed.
...
%	io:format(user, " value=~256p~n", [Value5]),
	...
	Value6 = mqtt_output:packet(suback, {[0,1,2], 102}),
...
%	io:format(user, "~n value=~256p~n", [Value12]),
	...
	Value13 = mqtt_output:packet(pubrec, 23045),
...
input_parser() ->
	...
	?assertEqual({suback, 103, [7], <<1:8, 1:8>>}, 
							 mqtt_input:input_parser(<<16#90:8, 3:8, 103:16, 7:8, 1:8, 1:8>>)),
...
encode_remaining_length() ->
	?assertEqual(<<45>>, mqtt_output:encode_remaining_length(45)),
	?assertEqual(<<161,78>>, mqtt_output:encode_remaining_length(10017)),
	?assertEqual(<<142,145,82>>, mqtt_output:encode_remaining_length(1345678)),
	?assertEqual(<<206,173,133,85>>, mqtt_output:encode_remaining_length(178345678)),
  ?passed.
