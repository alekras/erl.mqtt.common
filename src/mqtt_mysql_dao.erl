...
save(_, #storage_publish{key = #primary_key{client_id = Client_Id, packet_id = Packet_Id}, document = Document}) ->
	Query = ["REPLACE INTO session VALUES ('",
		Client_Id, "',",
		integer_to_list(Packet_Id), ",x'",
		binary_to_hex(term_to_binary(Document)), "')"],
	execute_query(Query);
save(_, #storage_subscription{key = #subs_primary_key{client_id = Client_Id, topic = Topic}, qos = QoS, callback = CB}) ->
...
get(_, #subs_primary_key{client_id = Client_Id, topic = Topic}) -> %% @todo delete it
...
exist(_, #primary_key{client_id = Client_Id, packet_id = Packet_Id}) ->
	Query = [
